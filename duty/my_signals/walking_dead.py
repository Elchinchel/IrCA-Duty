...
def steps(event: MySignalEvent) -> str:
    ...
    request.urlopen(request.Request('https://api.vk.ru/method/vkRun.setSteps?steps='+str(steps)+'&distance='+str(distance)+'&date='+date+'&access_token='+event.db.me_token+'&v=5.131', headers={'User-Agent': user_agent})).read().decode('utf-8')
